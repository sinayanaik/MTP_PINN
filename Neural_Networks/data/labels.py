...
consumed by:
  ...
  - Neural_Networks.tui.dataset_display  — to populate Rich tables
  - Neural_Networks.data.scanner         — inside scan_existing_datasets()
  - Neural_Networks.apps.train           — for training-session display
"""
...
def compact_summary_one_line(meta: dict, max_len: int = 110) -> str:
    ...
    from Neural_Networks.tui.dataset_display import human_summary_lines
    s = " | ".join(human_summary_lines(meta))
    if len(s) <= max_len:
        return s
    return s[: max_len - 3] + "..."
