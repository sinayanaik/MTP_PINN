#!/usr/bin/env python3
"""
TikZ Architecture Diagrams — 6 Publication-Quality Figures
==========================================================
...
% ── Node styles ───────────────────────────────────────────────────────
\tikzset{
  base/.style={draw, thick, rounded corners=3pt, align=center,
               font=\small, text=darktext, minimum height=0.9cm},
  ...
  headerbox/.style={draw=headeredge, thick, fill=headerfill,
                    rounded corners=4pt, minimum width=14cm,
                    minimum height=1.3cm, align=center, font=\large},
  arr/.style={->, >=stealth', thick, color=darktext},
  darr/.style={->, >=stealth', thick, dashed, color=darktext},
  eqnode/.style={align=center, font=\small, text=darktext},
  note/.style={align=left, font=\scriptsize, text=darktext},
}
"""
...
\node[headerbox] (hdr) {%
  ...
  {\small Pure data-driven baseline $\mid$ No physics information}};

% ── Input ───────────────────────────────────────────────────────────
\node[inputbox, minimum width=3.2cm, below=1.2cm of hdr.south west,
      anchor=north west, xshift=0.5cm]
  (inp) {%
    \textbf{Input}\\[2pt]
...
% ── MLP ─────────────────────────────────────────────────────────────
\node[netbox, minimum width=5.5cm, right=1.6cm of inp] (mlp) {%
  \textbf{MLP Backbone}\\[2pt]
  $15 \to [256,\, 512,\, 256] \to 5$\\[2pt]
  {\footnotesize Xavier normal init}};

% ── Output ──────────────────────────────────────────────────────────
\node[outputbox, minimum width=2.8cm, right=1.6cm of mlp] (out) {%
  \textbf{Output}\\[2pt]
...
% ── Hidden-layer detail ────────────────────────────────────────────
\node[eqnode, below=1.4cm of mlp, minimum width=13cm] (layer) {%
  \fbox{\parbox{12.5cm}{\centering
    \textbf{Each hidden layer ($i = 0, 1, 2$):}\\[4pt]
...
% ── Loss ────────────────────────────────────────────────────────────
\node[lossbox, below=1.2cm of layer, minimum width=14cm] (loss) {%
  $\displaystyle \mathcal{L}_A
    ...
    \quad {\footnotesize (joint\,2 shoulder upweighted)}};

% ── Note ────────────────────────────────────────────────────────────
\node[note, below=0.3cm of loss.south west, anchor=north west] {%
  Optimiser: AdamW, $\eta = 3\!\times\!10^{-4}$ \quad
  Gradient clip: $\|\nabla\|_{\max} = 5.0$ \quad
  Feature noise: $\sigma = 0.02$ \quad
  Batch size: 512};
...
\node[headerbox] (hdr) {%
  ...
  {\small Forward pass identical to Model~A $\mid$
   Physics enters \textit{only} through the training loss}};

% ═══ DATA PATH (top) ═══════════════════════════════════════════════
\node[inputbox, minimum width=3cm,
      below=1.0cm of hdr.south west, anchor=north west, xshift=0.3cm]
  (inp) {\textbf{Input}\\$\mathbf{x}\in\mathbb{R}^{15}$};

\node[netbox, minimum width=5cm, right=1.2cm of inp] (mlp) {%
  \textbf{MLP} (identical to A)\\[2pt]
  $15\!\to\![256, 512, 256]\!\to\!5$\\
  {\footnotesize SiLU $\mid$ Dropout(0.05) $\mid$ LayerNorm}};

\node[outputbox, minimum width=2.6cm, right=1.2cm of mlp] (out) {%
  $\hat{\bm{\tau}}\in\mathbb{R}^{5}$};
...
% data-loss label
\node[eqnode, below=0.5cm of out, xshift=-0.3cm] (dlabel) {%
  $\mathcal{L}_{\mathrm{data}}
   = \frac{1}{N}\sum_i \sum_j w_j
     (\hat{\tau}_{i,j} - \tau_{i,j}^{\mathrm{meas}})^2$};

% ═══ PHYSICS PATH (bottom) ════════════════════════════════════════
\node[physbox, minimum width=3.4cm,
      below=3.0cm of inp] (rnea) {%
  \textbf{RNEA precomputed}\\[2pt]
  $\bm{\tau}_g, \bm{\tau}_M, \bm{\tau}_C, \bm{\tau}_f$\\
  $\in\mathbb{R}^{20}$};

\node[opbox, minimum width=1.6cm, right=1.0cm of rnea] (sum) {%
  $\displaystyle\sum$\\[2pt]
  $\bm{\tau}_{\mathrm{eq}}\!\in\!\mathbb{R}^{5}$};

\draw[arr] (rnea) -- (sum);

\node[calibbox, minimum width=5cm, right=1.0cm of sum] (calib) {%
  \textbf{Calibration} $\varphi$ {\footnotesize(separate param group)}\\[2pt]
  $\varphi(\bm{\tau}) = \mathrm{diag}(\mathbf{s})\,\bm{\tau} + \mathbf{b}$\\[2pt]
  $s_j = \mathrm{softplus}(z_j) + 10^{-5}$\\[2pt]
  {\footnotesize Init: $\mathbf{s}=\mathbf{1},\;\mathbf{b}=\mathbf{0}$
   \quad $\lambda_{\mathrm{wd}}^{\varphi}=0$ (no weight decay)}};

\draw[arr] (calib.east) -- ++(1.0,0)
  node[outputbox, minimum width=2.2cm, anchor=west] (phyout) {%
  $\bm{\tau}_{\mathrm{phys}}^{\mathrm{cal}}$\\$\in\mathbb{R}^{5}$};

% ── Calibration regulariser ─────────────────────────────────────
\node[eqnode, below=0.6cm of calib] (creg) {%
  $\mathcal{L}_{\mathrm{calib}}
   = \tfrac{1}{J}\sum_j\bigl[(s_j - 1)^2 + b_j^2\bigr]$};

% ── Physics loss ────────────────────────────────────────────────
\node[eqnode, below=0.6cm of phyout, xshift=-0.5cm] (ploss) {%
...
\draw[darr] (out.south) -- ++(0,-1.6) -| (ploss.north);

% ── Weight schedule ─────────────────────────────────────────────
\node[eqnode, below=0.8cm of creg, xshift=-1cm] (wsched) {%
  \fbox{\parbox{10cm}{\centering
    $w_p(e) = 0.10 \cdot \min\!\bigl(1,\;\tfrac{e}{0.03\,E}\bigr)$
    \qquad $w_d = 1 - w_p$
    \qquad $\kappa = \hat{\mu}_d\,/\,\hat{\mu}_p$
    \quad {\footnotesize(EMA $\beta\!=\!0.98$)}
  }}};

% ── Total loss ──────────────────────────────────────────────────
\node[lossbox, below=0.7cm of wsched, minimum width=14cm] (total) {%
  $\displaystyle \mathcal{L}_B
   = w_d\,\mathcal{L}_{\mathrm{data}}
   + w_p\,\kappa\,\mathcal{L}_{\mathrm{phys}}$
  \qquad {\footnotesize Physics weight: ramp then CONSTANT
...
\node[headerbox] (hdr) {%
  ...
  {\small Learns state-dependent scaling $\bm{\alpha}$ and additive
   correction $\bm{\delta}$ on physics torque}};

% ── Inputs ──────────────────────────────────────────────────────────
\node[inputbox, minimum width=3cm,
      below=1.0cm of hdr.south west, anchor=north west, xshift=0.3cm]
  (feat) {\textbf{Features}\\$\mathbf{x}\in\mathbb{R}^{15}$};

\node[physbox, minimum width=3cm, below=0.8cm of feat]
  (phys) {\textbf{Physics (RNEA)}\\$\bm{\tau}_{\mathrm{phys}}\in\mathbb{R}^{5}$};

% ── Concat ──────────────────────────────────────────────────────────
\node[opbox, minimum width=2.4cm, right=1.8cm of feat, yshift=-0.9cm]
  (cat) {\textbf{Concat}\\$[\mathbf{x},\,\bm{\tau}_{\mathrm{phys}}]
          \in\mathbb{R}^{20}$};
...
% ── Encoder ─────────────────────────────────────────────────────────
\node[netbox, minimum width=5cm, right=1.4cm of cat] (enc) {%
  \textbf{Encoder MLP}\\[2pt]
  $20\!\to\![256, 512, 256]$\\
  {\footnotesize tanh $\mid$ Dropout(0.05) $\mid$ LayerNorm}\\
  $\mathbf{h}\in\mathbb{R}^{256}$};

\draw[arr] (cat) -- (enc);

% ── Dual heads ──────────────────────────────────────────────────────
\node[spdbox, minimum width=4.8cm,
      above right=1.4cm and 1.6cm of enc.east, anchor=west]
  (alpha) {%
  \textbf{$\bm{\alpha}$-head}\\[2pt]
  $\bm{\alpha} = \mathrm{softplus}(W_\alpha\mathbf{h}+\mathbf{b}_\alpha)
   + 10^{-3}$\\[2pt]
  {\footnotesize Init: $W_\alpha\!=\!0$,\;
   $\mathbf{b}_\alpha\!=\!\ln(e^{0.5}\!-\!1)\!\approx\!-0.481$
   $\;\Rightarrow\;\bm{\alpha}\!\approx\!0.5$}};

\node[fricbox, minimum width=4.2cm,
      below right=1.4cm and 1.6cm of enc.east, anchor=west]
  (delta) {%
  \textbf{$\bm{\delta}$-head}\\[2pt]
  $\bm{\delta} = W_\delta\mathbf{h} + \mathbf{b}_\delta$\\[2pt]
  {\footnotesize Init: $W_\delta\!=\!0,\;\mathbf{b}_\delta\!=\!0$
   $\;\Rightarrow\;\bm{\delta}\!=\!\mathbf{0}$}};

\draw[arr] (enc.east) -- ++(0.5,0) |- (alpha.west);
\draw[arr] (enc.east) -- ++(0.5,0) |- (delta.west);

% ── Output assembly ────────────────────────────────────────────────
\node[outputbox, minimum width=4cm,
      below=4.5cm of alpha.south, xshift=0.5cm] (out) {%
  \textbf{Output}\\[4pt]
  $\hat{\bm{\tau}} = \bm{\alpha}\odot\bm{\tau}_{\mathrm{phys}}
   + \bm{\delta}$};

\draw[arr] (alpha.south) -- ++(0,-1.0) -| (out.north west);
\draw[arr] (delta.south) -- ++(0,-0.6) -| (out.north east);

% physics tau also feeds output
\draw[darr, color=physgreenedge] (phys.south) -- ++(0,-6.8)
  -| (out.south);

% ── Warm-start note ─────────────────────────────────────────────
\node[eqnode, below=0.7cm of out] (init) {%
  \fbox{\parbox{11cm}{\centering
    \textbf{At $t\!=\!0$:}\quad
...
% ── Header ──────────────────────────────────────────────────────────
\node[headerbox, minimum width=16cm] (hdr) {%
  \textbf{Model D --- Lagrangian Structured FNN}\\[-2pt]
  {\small Equation-of-motion structure embedded in architecture:
   $\hat{\bm{\tau}} = M(q)\ddot{q} + C(q,\dot{q}) + g(q) + f(\dot{q})$}};

% ── Input ───────────────────────────────────────────────────────────
\node[inputbox, minimum width=3cm,
      below=6.0cm of hdr.south west, anchor=east, xshift=2.5cm]
  (inp) {\textbf{Input}\\$\mathbf{x}\!=\![q,\dot{q},\ddot{q}]$\\
          $\in\mathbb{R}^{15}$};

% ═══ Four sub-networks ═════════════════════════════════════════════
% M-net
\node[netbox, minimum width=5.5cm,
      below=1.2cm of hdr.south west, anchor=north west, xshift=2.8cm]
  (mnet) {%
  \textbf{M-net}\quad $q\in\mathbb{R}^{5}\to [256, 512, 256]\to\mathbb{R}^{15}$\\[2pt]
  \footnotesize tanh $\mid$ Dropout(0.05) $\mid$ LayerNorm};

\node[spdbox, minimum width=5.5cm, right=1.0cm of mnet] (spd) {%
  \textbf{Cholesky SPD enforcement}\\[2pt]
  $L_{\mathrm{diag}} = \mathrm{softplus}(L_{\mathrm{raw}}) + 10^{-4}$\\[2pt]
  $M = LL^\top + 10^{-4}\,I_5$\\[2pt]
  {\footnotesize Init: diag bias $= -2.0$
   $\;\Rightarrow\;\mathrm{softplus}(-2)\!\approx\!0.126$}};

\draw[arr] (mnet) -- (spd);

\node[opbox, minimum width=2.6cm, right=1.0cm of spd]
  (matmul) {$\bm{\tau}_M = M\ddot{q}$};

\draw[arr] (spd) -- (matmul);

% C-net
\node[netbox, minimum width=5.5cm, below=0.8cm of mnet]
  (cnet) {%
  \textbf{C-net}\quad $[q,\dot{q}]\in\mathbb{R}^{10}\to [256, 512, 256]\to\mathbb{R}^{5}$\\[2pt]
  \footnotesize tanh $\mid$ Dropout(0.05) $\mid$ LayerNorm};

\node[opbox, minimum width=2.2cm] at (matmul |- cnet)
  (cout) {$\bm{\tau}_C$};

\draw[arr] (cnet) -- (cout);

% g-net
\node[netbox, minimum width=5.5cm, below=0.8cm of cnet]
  (gnet) {%
  \textbf{g-net}\quad $q\in\mathbb{R}^{5}\to [256, 512, 256]\to\mathbb{R}^{5}$\\[2pt]
  \footnotesize tanh $\mid$ Dropout(0.05) $\mid$ LayerNorm};

\node[opbox, minimum width=2.2cm] at (matmul |- gnet)
  (gout) {$\bm{\tau}_g$};

\draw[arr] (gnet) -- (gout);

% f-net
\node[fricbox, minimum width=5.5cm, below=0.8cm of gnet]
  (fnet) {%
  \textbf{f-net}\quad $\dot{q}\in\mathbb{R}^{5}\to [128, 128]\to\mathbb{R}^{10}$\\[2pt]
  \footnotesize tanh $\mid$ Dropout(0.05)\\[2pt]
  {\footnotesize viscous ($c_v$) + Coulomb ($c_c$)}};

\node[fricbox, minimum width=5.5cm, right=1.0cm of fnet] (fdiss) {%
  \textbf{Dissipative friction}\\[2pt]
  $\bm{\tau}_f = -\bigl[\mathrm{sp}(c_v)\dot{q}
    + \mathrm{sp}(c_c)\tanh(\dot{q}/0.04)\bigr]$\\[2pt]
  {\footnotesize $\bm{\tau}_f\!\cdot\!\dot{q} \le 0$ by construction}};

\draw[arr] (fnet) -- (fdiss);

\node[opbox, minimum width=2.2cm] at (matmul |- fdiss)
  (fout) {$\bm{\tau}_f$};

\draw[arr] (fdiss) -- (fout);

% ── Input arrows ────────────────────────────────────────────────
\draw[arr] (inp.north) -- ++(0,0.5) -| (mnet.west);
\draw[arr] (inp.east) -- ++(0.5,0) |- (cnet.west);
\draw[arr] (inp.east) -- ++(0.5,0) |- (gnet.west);
\draw[arr] (inp.south) -- ++(0,-0.5) -| (fnet.west);

% ── Summation ───────────────────────────────────────────────────
\node[outputbox, minimum width=3.8cm,
      below=1.4cm of fout] (sum) {%
  \textbf{Output}\\[2pt]
  $\hat{\bm{\tau}} = \bm{\tau}_M + \bm{\tau}_C + \bm{\tau}_g + \bm{\tau}_f$};

\draw[arr] (matmul.south) -- ++(0,-0.4) -| ([xshift=-0.8cm]sum.north);
\draw[arr] (cout.south) -- ++(0,-0.4) -| ([xshift=-0.3cm]sum.north);
\draw[arr] (gout.south) -- ++(0,-0.4) -| ([xshift=0.3cm]sum.north);
\draw[arr] (fout.south) -- (fout |- sum.north);

% ── Loss ────────────────────────────────────────────────────────
\node[lossbox, below=1.2cm of sum, minimum width=16cm] (loss) {%
  ...
  \quad {\footnotesize $\epsilon_{\mathrm{SPD}}=10^{-4}$}};
...
\node[headerbox, minimum width=16cm] (hdr) {%
  ...
  {\small Equation residual loss $\mid$ Collocation at synthetic states
   $\mid$ LossNormaliser $\kappa$}};

% ═══ PATH 1: DATA (top) ════════════════════════════════════════════
\node[inputbox, minimum width=2.8cm,
      below=1.0cm of hdr.south west, anchor=north west, xshift=0.3cm]
  (inp) {\textbf{Input}\\$\mathbf{x}\in\mathbb{R}^{15}$};

\node[netbox, minimum width=5.0cm, right=1.2cm of inp] (mlp) {%
  \textbf{MLP} (identical to A)\\[2pt]
  $15\!\to\![256, 512, 256]\!\to\!5$\\
  {\footnotesize SiLU $\mid$ Dropout(0.05) $\mid$ LayerNorm}};

\node[outputbox, minimum width=2.4cm, right=1.2cm of mlp] (tauhat) {%
  $\hat{\bm{\tau}}\in\mathbb{R}^{5}$};
...
\node[physbox, minimum width=3.0cm, below=2.0cm of inp] (rnea) {%
  \textbf{RNEA precomputed}\\[2pt]
  $\bm{\tau}_g, \bm{\tau}_M, \bm{\tau}_C, \bm{\tau}_f$\\
  $\in\mathbb{R}^{20}$};

\node[opbox, minimum width=1.6cm, right=0.8cm of rnea] (sum) {%
  $\displaystyle\sum$\\
  $\bm{\tau}_{\mathrm{nom}}\!\in\!\mathbb{R}^{5}$};

\draw[arr] (rnea) -- (sum);

\node[calibbox, minimum width=5.2cm, right=0.8cm of sum] (calib) {%
  \textbf{Calibration} $\varphi$ {\footnotesize(separate param group)}\\[2pt]
  $\varphi(\bm{\tau}) = \mathrm{diag}(\mathbf{s})\,\bm{\tau} + \mathbf{b}$\\[2pt]
  $s_j = \mathrm{softplus}(z_j) + 10^{-5}$\\[1pt]
  {\footnotesize Init: $\mathbf{s}\!=\!\mathbf{1},\;\mathbf{b}\!=\!\mathbf{0}$
   \quad $\lambda_{\mathrm{wd}}^{\varphi}\!=\!0$}};

\draw[arr] (sum) -- (calib);

\node[outputbox, minimum width=2.2cm, right=0.8cm of calib] (phyout) {%
  $\bm{\tau}_{\mathrm{phys}}^{\mathrm{cal}}$\\$\in\mathbb{R}^{5}$};

\draw[arr] (calib) -- (phyout);

% ── Equation residual ──────────────────────────────────────────
\node[eqnode, right=0.6cm of phyout, yshift=1.2cm] (resid) {%
  \fbox{\parbox{4.2cm}{\centering
    \textbf{Equation residual}\\[3pt]
    $\mathbf{r} = \hat{\bm{\tau}} - \varphi(\bm{\tau}_{\mathrm{nom}})$\\[3pt]
    $\mathcal{L}_{\mathrm{phys}} = \|\mathbf{r}\|^2
     + 0.01\,\mathcal{L}_{\mathrm{calib}}$
  }}};

\draw[darr] (tauhat.south) -- ++(0,-0.5) -| (resid.north);
\draw[darr] (phyout.north) -- ++(0,0.5) -| (resid.south);

% ── Calibration regulariser ─────────────────────────────────────
\node[eqnode, below=0.7cm of calib] (creg) {%
  $\mathcal{L}_{\mathrm{calib}}
   = \tfrac{1}{J}\sum_j[(s_j\!-\!1)^2 + b_j^2]$
  \;\; --- keeps $\varphi$ near identity};

% ═══ PATH 3: COLLOCATION (bottom) ══════════════════════════════════
\node[collocbox, minimum width=3.6cm, below=2.0cm of rnea] (coll) {%
  \textbf{Collocation sampling}\\[2pt]
  $n_{\mathrm{col}} = 32$ per epoch\\[1pt]
...
\node[physbox, minimum width=3.2cm, right=0.8cm of coll] (collrnea) {%
  \textbf{RNEA + friction}\\
  {\footnotesize (Pinocchio, physical units)}\\
  $\to$ z-score normalise};

\draw[arr] (coll) -- (collrnea);

\node[calibbox, minimum width=3.0cm, right=0.8cm of collrnea] (collcalib) {%
  \textbf{Calibration} $\varphi$ {\footnotesize(shared)}\\
  same params as above};

\draw[arr] (collrnea) -- (collcalib);

\node[eqnode, right=0.8cm of collcalib] (colloss) {%
  $\mathcal{L}_{\mathrm{col}} = \|\mathbf{r}_{\mathrm{col}}\|^2$\\
  $\lambda_{\mathrm{col}} = 0.05$};

\draw[arr] (collcalib) -- (colloss);

% ── Collocation note ────────────────────────────────────────────
\node[note, below=0.3cm of coll.south west, anchor=north west] {%
  Collocation extends physics supervision into
  $\mathcal{U}[\mu \pm 3\sigma]$ --- beyond training distribution};

% ── Weight schedule ─────────────────────────────────────────────
\node[eqnode, below=3.2cm of creg, xshift=0.5cm] (wsched) {%
  $w_p(e) = 0.10\cdot\min\!\bigl(1,\;\tfrac{e}{0.03\,E}\bigr),$
  \quad $w_d = 1 - w_p,$
  \quad $\kappa = \hat{\mu}_d / \hat{\mu}_p$
  \;\;{\footnotesize ($\beta=0.98$)}};

% ── Total loss ──────────────────────────────────────────────────
\node[lossbox, below=0.7cm of wsched, minimum width=16cm] (total) {%
...
\node[headerbox, minimum width=16cm] (hdr) {%
  ...
  {\small Correction networks on RNEA nominal $\mid$ Warm-start init
   $\mid$ Occam regulariser on $\Delta$}};

% ── Input ───────────────────────────────────────────────────────────
\node[inputbox, minimum width=2.8cm,
      below=6.6cm of hdr.south west, anchor=east, xshift=2.5cm]
  (inp) {\textbf{Input}\\$\mathbf{x}\!=\![q,\dot{q},\ddot{q}]$\\
          $\in\mathbb{R}^{15}$};

\node[physbox, minimum width=3.0cm, below=1.0cm of inp]
  (nom) {\textbf{RNEA nominal}\\[1pt]
  $\bm{\tau}_{g}^{\mathrm{nom}},\,\bm{\tau}_{C}^{\mathrm{nom}},\,
   \bm{\tau}_{f}^{\mathrm{nom}}$\\
  $\in\mathbb{R}^{20}$};

% ═══ M-net (from scratch) ═════════════════════════════════════════
\node[netbox, minimum width=5.2cm,
      below=1.2cm of hdr.south west, anchor=north west, xshift=2.8cm] (mnet) {%
  \textbf{M-net} (from scratch)\\[2pt]
  $q\!\in\!\mathbb{R}^{5}\to [256, 512, 256]\to\mathbb{R}^{15}$\\
  {\footnotesize tanh $\mid$ Dropout(0.05) $\mid$ LayerNorm}};

\node[spdbox, minimum width=4.8cm, right=1.0cm of mnet] (spd) {%
  \textbf{Cholesky $\to$ SPD}\\[2pt]
  $L_{\mathrm{diag}}\!=\!\mathrm{softplus}(L_{\mathrm{raw}})+10^{-4}$\\
  $M\!=\!LL^\top+10^{-4}I$\\[1pt]
  {\footnotesize diag bias $=-2.0$}};

\draw[arr] (mnet) -- (spd);

\node[opbox, right=1.0cm of spd] (mmul) {$\bm{\tau}_M\!=\!M\ddot{q}$};
\draw[arr] (spd) -- (mmul);

% ═══ C-net (correction) ═══════════════════════════════════════════
\node[netbox, minimum width=5.2cm, below=0.8cm of mnet] (cnet) {%
  \textbf{c-net} (correction)\\[2pt]
  $[q,\dot{q}]\!\in\!\mathbb{R}^{10}\to [256, 512, 256]\to\Delta\mathbf{c}$\\
  {\footnotesize tanh $\mid$ Dropout(0.05) $\mid$ LayerNorm}};

\node[opbox, minimum width=3.6cm] at (mmul |- cnet) (cadd) {%
  $\bm{\tau}_C = \bm{\tau}_{C}^{\mathrm{nom}} + \Delta\mathbf{c}$};

\draw[arr] (cnet) -- (cadd);

% ═══ g-net (correction) ═══════════════════════════════════════════
\node[netbox, minimum width=5.2cm, below=0.8cm of cnet] (gnet) {%
  \textbf{g-net} (correction)\\[2pt]
  $q\!\in\!\mathbb{R}^{5}\to [256, 512, 256]\to\Delta\mathbf{g}$\\
  {\footnotesize tanh $\mid$ Dropout(0.05) $\mid$ LayerNorm}};

\node[opbox, minimum width=3.6cm] at (mmul |- gnet) (gadd) {%
  $\bm{\tau}_g = \bm{\tau}_{g}^{\mathrm{nom}} + \Delta\mathbf{g}$};

\draw[arr] (gnet) -- (gadd);

% ═══ f-net (correction, dissipative) ══════════════════════════════
\node[fricbox, minimum width=5.2cm, below=0.8cm of gnet] (fnet) {%
  \textbf{f-net} (correction)\\[2pt]
  $\dot{q}\!\in\!\mathbb{R}^{5}\to [128, 128]\to\mathbb{R}^{10}$\\
  {\footnotesize tanh $\mid$ Dropout(0.05)}};

\node[fricbox, minimum width=5.5cm, right=1.0cm of fnet] (fdiss) {%
  \textbf{Dissipative correction}\\[2pt]
  $\Delta\mathbf{f} = -\bigl[\mathrm{sp}(c_v)\dot{q}
    + \mathrm{sp}(c_c)\tanh(\dot{q}/0.04)\bigr]$\\[1pt]
...
% nominal arrows — route below all sub-nets then up on the right
\draw[darr, color=physgreenedge] (nom.east) -- ++(1.0,0) -| (cadd.south);
\draw[darr, color=physgreenedge] (nom.east) -- ++(1.0,0) -| (gadd.south);
\draw[darr, color=physgreenedge] (nom.south) -- ++(0,-0.5) -| (fdiss.south);

% ── Summation / Output ─────────────────────────────────────────
\node[outputbox, minimum width=4.5cm,
      below=1.4cm of fdiss] (out) {%
  \textbf{Output}\\[3pt]
  $\hat{\bm{\tau}} = \bm{\tau}_M + \bm{\tau}_C + \bm{\tau}_g + \bm{\tau}_f$};

\draw[arr] (mmul.south) -- ++(0,-0.4) -| ([xshift=-1.2cm]out.north);
\draw[arr] (cadd.south) -- ++(0,-0.4) -| ([xshift=-0.4cm]out.north);
\draw[arr] (gadd.south) -- ++(0,-0.4) -| ([xshift=0.4cm]out.north);
\draw[arr] (fdiss.south) -- ++(0,-0.4) -| ([xshift=1.2cm]out.north);

% ── Warm-start note ─────────────────────────────────────────────
\node[eqnode, below=1.0cm of out] (warm) {%
  \fbox{\parbox{14cm}{\centering
    \textbf{Warm-start initialisation:}\;
    Last-layer weights $W\sim\mathcal{N}(0,\,10^{-3})$,\;
    biases $\mathbf{b}=\mathbf{0}$\\[3pt]
...
% ── Loss ────────────────────────────────────────────────────────
\node[lossbox, below=0.8cm of warm, minimum width=16cm] (loss) {%
