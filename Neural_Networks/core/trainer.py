...
are emitted by the caller (train.py) using the `console` object from
``Neural_Networks.tui.console``.  The ``train_model`` function accepts an
optional ``console`` keyword argument so the caller can inject its own
Console instance; if omitted a plain stdlib logger is used instead.
...
from Neural_Networks.core.metrics import compute_metrics, trajectory_mean_rmse_numpy, macro_rmse_numpy
from Neural_Networks.core.builder import build_model, build_optimizer, build_scheduler
...
from Neural_Networks.models import (
    ...
    LAGRANGIAN_MODELS, MODEL_SAVE_DIRS,
    PHYSICS_INPUT_MODELS, PHYSICS_WEIGHT_MODELS,
)
from Neural_Networks.models.unified_abc_fnn import _reduce_physics_to_total
...
def _split_decomposed_physics(
    physics: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Split the 20-dim physics tensor into (tau_g, tau_M, tau_C, tau_f) components.

    Expects exactly 4 * ACTIVE_JOINTS dims [tau_g(5), tau_M(5), tau_C(5), tau_f(5)].
    Raises ValueError for any other shape — rebuild the dataset if this occurs.

    Returns (tau_g, tau_M, tau_C, tau_f) all shape (..., n_joints).
    """
    n = physics.shape[-1]
    if n != 4 * ACTIVE_JOINTS:
        raise ValueError(
            f"Physics tensor last dim {n} != {4 * ACTIVE_JOINTS}. "
            "Rebuild the dataset with the current preprocessor."
        )
    nj = ACTIVE_JOINTS
    tau_g = physics[..., :nj]
    tau_M = physics[..., nj:2*nj]
    tau_C = physics[..., 2*nj:3*nj]
    tau_f = physics[..., 3*nj:]
    return tau_g, tau_M, tau_C, tau_f
...
def _forward_pass(
    ...
    Returns (tau_hat, components) where components is a dict (may be empty).
    Decomposed models return both tau_hat and a component dict containing
    ``M`` and ``tau_f`` tensors needed for the SPD / friction loss terms.
    """
    if model_type in DECOMPOSED_MODELS:
...
def _compute_loss(
    ...
    Per model
    ---------
    - BlackBox                : L_data only (α irrelevant).
    ...
    - EquationConstrainedPINN : (1−α)·L_data + α·L_phys_res  (α-mixture active)
    - LagrangianStructured    : L_data + spd_w·L_spd + friction_w·L_fr
                                (physics in architecture; α ignored)
    - DecomposedStructuredPINN: as Lagrangian + correction_reg·L_corr
                                (physics in architecture; α ignored)

    Why no α-mixture on structured models?
    --------------------------------------
    Their physics is the *forward pass* (SPD Cholesky inertia, decomposed
    RNEA-style sum, dissipative friction).  Adding "match τ_RNEA" as a loss
    anchor only pulls predictions toward the biased nominal model and degrades
    val RMSE — the calibrated τ_nom has MASS_SCALE + friction residual error
    that competes with the cleaner τ_meas signal.
    ...
    if model_type in EQUATION_CONSTRAINED_MODELS:
        tau_g, tau_M, tau_C, tau_f = _split_decomposed_physics(physics)
        phys_loss = _loss_model.compute_loss(
            tau_hat, target, tau_M, tau_C, tau_g, tau_f,
        )["physics"]
        if _norm is not None:
            _norm.update(float(data_loss.detach().item()), float(phys_loss.detach().item()))
            kappa = _norm.scale()
        else:
            kappa = 1.0
        return w_data * data_loss + w_phys * kappa * phys_loss

    if model_type in DECOMPOSED_MODELS:
        # Decomposed structured PINN:  τ̂ = M̂(q)q̈ + ĉ(q,q̇) + ĝ(q) + f̂(q̇).
        # The physics is the ARCHITECTURE (SPD Cholesky + dissipative friction +
        # decomposed RNEA corrections).  We do NOT add a "match τ_RNEA" anchor
        # loss — it would only drag predictions toward the biased nominal model
        # (MASS_SCALE error, imperfect friction constants) and cost PINN quality.
        # Structural priors (SPD, friction dissipation) and the correction Occam
        # regulariser stay as fixed-weight additions.  α (physics_weight) has
        # no effect on this model — the physics already lives in the forward
        # pass.
        if features is None:
            raise ValueError("features tensor is required for DecomposedStructuredPINNFNN loss")
        qd    = features[..., 5:10]
        M_hat = components.get("M")
        tau_f = components.get("tau_f")
        loss_dict = _loss_model.compute_loss(
            tau_hat, target, qd, M_hat, tau_f,
            external_data_loss=data_loss,
            components=components,
            tau_physics_nom=None,   # anchor explicitly disabled
        )
        spd_w      = float(hp.get("spd_weight",            0.01))
        friction_w = float(hp.get("friction_weight",       0.01))
        corr_w     = float(hp.get("correction_reg_weight", 0.001))
        spd_t  = loss_dict.get("spd",        data_loss.new_zeros(()))
        fr_t   = loss_dict.get("friction",   data_loss.new_zeros(()))
        corr_t = loss_dict.get("correction", data_loss.new_zeros(()))
        return (data_loss
                + spd_w * spd_t + friction_w * fr_t
                + corr_w * corr_t)

    if model_type in LAGRANGIAN_MODELS:
        # Lagrangian-structured PINN:  τ̂ = M̂(q)q̈ + ĉ(q,q̇) + ĝ(q) + f̂(q̇).
        # Same reasoning as Decomposed — physics lives in the architecture
        # (SPD + dissipative friction).  No RNEA anchor loss.  α has no effect.
        if features is None:
            return data_loss
        qd    = features[..., 5:10]
        M_hat = components.get("M")
        tau_f = components.get("tau_f")
        if M_hat is not None and tau_f is not None:
            loss_dict = _loss_model.compute_loss(
                tau_hat, target, qd, M_hat, tau_f,
                external_data_loss=data_loss,
            )
            ...
            spd_t  = loss_dict.get("spd",      data_loss.new_zeros(()))
            fr_t   = loss_dict.get("friction", data_loss.new_zeros(()))
            return data_loss + spd_w * spd_t + friction_w * fr_t
        return data_loss

    # BlackBoxFNN: data MSE only
    return data_loss
...
def train_model(
    ...
    *,
    ...
    for epoch in range(1, epochs + 1):
        ...
        # ── Optional collocation step (EC-PINN) ──────────────────────────────
        if _colloc_sampler is not None and model_type in EQUATION_CONSTRAINED_MODELS:
            ...
            if lc > 0.0 and n_c > 0:
                ...
                _phy_z = torch.zeros(feat_c.shape[0], ACTIVE_JOINTS,
                                     device=device, dtype=feat_c.dtype)
    ...
    from Neural_Networks.core.checkpoint_io import build_run_id
    ...
    from Neural_Networks.core.checkpoint_io import save_checkpoints
    ...
    from Neural_Networks.core.checkpoint_io import dump_yaml, exhaustive_hparams
