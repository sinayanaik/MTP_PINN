...
def build_model(model_type: str, hp: dict, device: torch.device) -> nn.Module:
    ...
    ----------
    model_type : str
        One of the 6 model type names in MODEL_REGISTRY.
    hp : dict
    ...
    if model_type in ("BlackBoxFNN", "PhysicsRegularizedFNN"):
        kwargs: dict = {
            "n_joints":      n_joints,
...
def build_optimizer(model: nn.Module, hp: dict) -> torch.optim.Optimizer:
    ...
    sub-module (EquationConstrainedPINNFNN) — the calibration layers gets
    a lower LR scaled by ``phi_lr_ratio``.
