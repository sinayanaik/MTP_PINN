...
def apply_style(cfg: PlotConfig) -> None:
    """Apply scienceplots + a Times-like serif theme parameterised by ``cfg``."""
    try:
        import scienceplots  # noqa: F401
        plt.style.use(["science", "no-latex"])
    except Exception:  # noqa: BLE001 - any failure -> graceful fallback
        logger.warning("scienceplots unavailable; falling back to default style.")
