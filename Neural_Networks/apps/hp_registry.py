...
Python dicts, sets, and (Config → dict) functions.  Interactive prompting for training lives in ``Neural_Networks.apps.train_ui``.
...
LAGRANGIAN_MODELS: set[str] = set()
DECOMPOSED_MODELS: set[str] = set()
EQUATION_CONSTRAINED_MODELS: set[str] = set()
...
PHYSICS_WEIGHT_HP: dict[str, dict[str, Any]] = {
    "phi_lr_ratio": {
        "default": 0.1,
        "desc": "Learning-rate multiplier for τ_eq calibration vs main network LR.",
        "effect": "Smaller LR for per-joint scale/bias calibration (PhysicsRegularizedFNN).",
    },
    "physics_weight": {
        "default": 0.10,
...
HP_KEY_GROUPS: dict[str, list[str]] = {
    ...
    "physics": [k for k in PHYSICS_WEIGHT_HP.keys()],
...
def apply_accurate_nominal_to_docs(
    ...
    *,
    ...
    if 0 < _n < 250_000:
        ...
        if model_type in FNN_MODELS and "dropout" in docs:
...
def get_default_hp(
    ...
    """Return default hyperparameters (no interactive prompts)."""
